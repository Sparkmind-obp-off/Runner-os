...
const runtimeFor = (db?: D1Database, githubToken?: string) => db
  ? createRunner({}, {}, new D1RunnerStore(db), { token: githubToken })
  : githubToken ? createRunner({}, {}, undefined, { token: githubToken }) : fallbackRuntime
...
app.post('/api/runs', async (c) => {
  ...
  const result = await runtimeFor(c.env?.DB, c.env?.GITHUB_TOKEN).engine.execute(body)
  return c.json(result, result.status === 'completed' ? 200 : result.status === 'awaiting_approval' ? 202 : 422)
})

app.get('/api/runs/:runId', async (c) => {
  const runId = c.req.param('runId')
  const store = runtimeFor(c.env?.DB, c.env?.GITHUB_TOKEN).store
...
app.post('/api/runs/:runId/approval', async (c) => {
  ...
  try {
    const result = await runtimeFor(c.env?.DB, c.env?.GITHUB_TOKEN).engine.decideApproval(c.req.param('runId'), body.decision, body.actor.trim(), typeof body.reason === 'string' ? body.reason : undefined)
    return c.json(result, result.status === 'completed' ? 200 : result.status === 'awaiting_approval' ? 202 : 422)
...
app.post('/api/runs/:runId/cancel', async (c) => {
  ...
  const accepted = await runtimeFor(c.env?.DB, c.env?.GITHUB_TOKEN).engine.cancel(c.req.param('runId'), typeof body.actor === 'string' && body.actor.trim() ? body.actor.trim() : 'api')
  return c.json({ accepted }, accepted ? 202 : 409)
})

app.post('/api/runs/:runId/recover', async (c) => {
  try {
    const result = await runtimeFor(c.env?.DB, c.env?.GITHUB_TOKEN).engine.recover(c.req.param('runId'), 'api-recovery')
    return c.json(result, result.status === 'completed' ? 200 : result.status === 'awaiting_approval' ? 202 : 422)
